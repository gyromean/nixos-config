...
vim.keymap.set({"n"}, "<leader>ss", function() require'school_notes'.start() end, {desc = "[S]chool notes [s]tart"})
vim.keymap.set({"n"}, "<leader>se", function() require'school_notes'.stop() end, {desc = "[S]chool notes [e]nd"})
vim.keymap.set({"n"}, "<leader>sn", function() require'school_notes'.normal_note() end, {desc = "[S]chool notes add [n]ormal note"})
vim.keymap.set({"n"}, "<leader>sm", function() require'school_notes'.minimal_note() end, {desc = "[S]chool notes add [m]inimal note"})
...
local trailing_whitespace = function()
  ...
  if vim.bo.buftype == "terminal" or vim.bo.filetype == "opencode_terminal" then
    return
  end

  vim.w.trailing_whitespace_match_id = vim.fn.matchadd("TrailingWhitespace", [[\s\+$]])
end
vim.api.nvim_create_autocmd({ "WinNew", "WinEnter", "BufWinEnter", "TermOpen", "FileType" }, {
