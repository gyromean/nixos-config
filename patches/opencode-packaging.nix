{ pkgs, inputs }:
let
  # Upstream v1.14.24 flake packaging is broken: the node_modules build excludes
  # the workspace root, so root devDependencies needed during build are missing.
  opencodePkgs = import inputs.opencode.inputs.nixpkgs {
    inherit (pkgs.stdenv.hostPlatform) system;
    overlays = [ inputs.opencode.overlays.default ];
  };
in
(opencodePkgs.callPackage "${inputs.opencode.outPath}/nix/node_modules.nix" {
  rev = "da6683f";
